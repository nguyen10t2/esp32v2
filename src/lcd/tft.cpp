...
void tft_draw_string(int x, int y, const char *str, uint16_t color) {
    // Thêm màu nền (Black) để xoá đi điểm ảnh cũ, tránh hiện tượng chữ đè lên nhau thành cục hình
    // vuông
...
void tft_display_direction(direction_t dir) {
    if (dir == DIR_OFF) {
        ...
        // Cản luôn Driver LCD xuất tín hiệu (Nhanh hơn fillScreen + tiết kiệm thêm 20mA của IC màn
        // hình) Command 0x28 (Display OFF) giúp màn hình tắt hiển thị mà không cần SPI gửi hàng
        // triệu pixel đen
