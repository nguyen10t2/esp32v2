...
void f5_mqtt_init(void) {
    ...
    if (esp_sleep_get_wakeup_cause() != ESP_SLEEP_WAKEUP_TIMER) {
        ...
        kalman_init(&mySmokeFilter, DEFAULT_KALMAN_ERROR, DEFAULT_KALMAN_ERROR,
                    DEFAULT_KALMAN_NOISE);
...
bool f5_mqtt_update(float raw_temp, float raw_smoke, bool raw_flame, uint8_t battery,
    ...
    //  ví dụ: có thể kết hợp nhiệt độ cao + khói nhiều nhưng chưa đủ để gọi là cháy thì vẫn là cảnh
    //  báo. Hoặc có thể thêm trạng thái NODE_ERROR nếu cảm biến có dấu hiệu lỗi (ví dụ: nhiệt độ âm
    //  hoặc khói âm). dựa trên nhiều yếu tố (ví dụ: nhiệt độ cao + khói nhiều nhưng chưa đủ để gọi
    //  là cháy)
